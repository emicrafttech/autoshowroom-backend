...
urlpatterns = [
    ...
    path("platform/reports/stats", StatsView.as_view(model=ContentReport), name="platform-report-stats"),
    path("platform/reports/stats/overdue", StatsView.as_view(model=ContentReport), name="platform-report-overdue-stats"),
    path("platform/dsr-requests/stats", StatsView.as_view(model=DataSubjectRequest), name="platform-dsr-stats"),
    path("platform/appeals/stats", StatsView.as_view(model=SanctionAppeal), name="platform-appeals-stats"),
    path("platform/security-incidents/stats", StatsView.as_view(model=SecurityIncident), name="platform-security-stats"),
    path("platform/watchlists/stats", StatsView.as_view(model=WatchlistEntry), name="platform-watchlist-stats"),
    path("platform/roles/stats", StatsView.as_view(model=PlatformRole, status_field="name"), name="platform-role-stats"),
    path("platform/users/stats", StatsView.as_view(model=StaffUser, status_field="is_active"), name="platform-user-stats"),
    ...
    path("platform/audit-trail/stats", StatsView.as_view(model=AuditLog, status_field="action"), name="platform-audit-stats"),
    ...
    path("platform/dealers/verification-queue/stats", StatsView.as_view(model=Dealer, status_field="verification_status"), name="platform-dealer-verification-stats"),
    path("platform/locations/premises-queue", DealerQueueView.as_view(queue_type="premises"), name="platform-premises-queue"),
    path("platform/locations/premises-queue/stats", StatsView.as_view(model=DealerLocation, status_field="premises_verification_status"), name="platform-premises-stats"),
