...
from apps.billing.limits import soft_inactivate_excess_listings
...
class June2026PlansTests(TestCase):
    ...
    def test_founding_trial_checkout_is_free(self):
        response = self.client.post(
            "/v1/billing/checkout",
            {"planId": "growth", "billingInterval": "monthly"},
            format="json",
        )
        ...
        complete = self.client.post(
            ...
            {"planId": "growth", "reference": payload["reference"]},
            format="json",
        )
        ...
        self.assertEqual(sub.plan_id, "growth")
        self.dealer.refresh_from_db()
        self.assertEqual(self.dealer.plan_id, "growth")
